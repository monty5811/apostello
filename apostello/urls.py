...
urlpatterns = [
    url(r'^$', v.SimpleView.as_view(template_name="apostello/index.html", required_perms=[]), name='index'),
    ...
    url(r'^send/adhoc/', v.SendAdhoc.as_view(required_perms=['can_send_sms']), name='send_adhoc'),
    url(r'^send/group/', v.SendGroup.as_view(required_perms=['can_send_sms']), name='send_group'),
]
# recipient group urls
urlpatterns += [
    url(r'^group/all/$', v.SimpleView.as_view(template_name='apostello/groups.html', required_perms=['can_see_groups']), name='recipient_groups'),
    url(r'^group/new/$',
        v.ItemView.as_view(
...
# recipient urls
urlpatterns += [
    url(r'^recipient/all/$', v.SimpleView.as_view(template_name='apostello/recipients.html', required_perms=['can_see_contact_names']), name='recipients'),
    url(r'^recipient/new/$',
        v.ItemView.as_view(
    ...
    url(r'^keyword/edit/(?P<pk>\d+)/$',
        keyword_access_check(
            v.ItemView.as_view(
                ...
                required_perms=['can_see_keywords'])),
        name='keyword'),
    url(r'^keyword/responses/(?P<pk>\d+)/$', v.keyword_responses, name='keyword_responses'),
    url(r'^keyword/responses/(?P<pk>\d+)/archive/$', v.keyword_responses, {'archive': True}, name='keyword_responses_archive'),
    url(r'^keyword/responses/csv/(?P<pk>\d+)/$', v.keyword_csv, name='keyword_csv'),
    url(r'^keyword/responses/wall/(?P<pk>\d+)/$', v.keyword_wall, name='keyword_wall'),
    url(r'^keyword/responses/curate_wall/(?P<pk>\d+)/$', v.keyword_wall_curate, name='keyword_wall_curator'),
]
...
# import urls
urlpatterns += [
    url(r'^recipient/import/$', v.import_recipients, name='import_recipients'),
    url(r'^elvanto/import/$', v.ElvantoImportView.as_view(required_perms=['can_import']), name='import_elvanto'),
]
